...
#reading in the data 
raw_data01 <- read.csv("/Users/alexlawson/Masters-Data-Final/Data-frames/final-dataframe_ciernia.csv", stringsAsFactors = FALSE)
View(raw_data01)
new_df <- raw_data01 %>%
  select(-PC1, -PC2, -PC3, -Cluster)
View(new_df)

fraclac_dir_live <- "/Users/alexlawson/Masters-Data-Final/Live-imaging/Ciernia-Lab-Morphometrics/Data/FracLac/20251202104933"
skeleton_dir_live <- "/Users/alexlawson/Masters-Data-Final/Live-imaging/Ciernia-Lab-Morphometrics/Data/SkeletonResults"
fraclac_live <- fraclac_tidying(fraclac_dir_live) 
skeleton_live <- skeleton_tidying(skeleton_dir_live)
data_livecell <- merge_data(fraclac_live, skeleton_live)

finaldata_live <- metadata_columns(data_livecell, c("BrainRegion", "MouseID","Treatment","Sex","SlideNumber"), sep="_")
names(finaldata_live) <- names(new_df)
View(finaldata_live)

#LIVE DATA MERGING
df_merged_log <- transform_log(finaldata_live,1, start=8, end=34) 
...
final_df_live <- cbind(pca_df_merged_log[1:3], finaldata_live, as.data.frame(data_kmeans_pca_df_merged_log_transformed$cluster)) %>%
...
plot_clusters <- clusterplots(final_df_live, "PC1", "PC2")
plot_clusters
final_df_live_withclustername <- final_df_live %>% mutate(Cluster = 
                                                                  case_when(Cluster=="1" ~ "RodLike",
                                                                            Cluster=="2" ~ "Ramified",
                                                                            Cluster=="3" ~ "Ameboid"))
write.csv(final_df_live_withclustername, "/Users/alexlawson/Masters-Data-Final/Live-imaging/Ciernia-Lab-Morphometrics/Results/livecelldataset_withclustername.csv", row.names = FALSE)



View(final_df_live_withclustername)
...
cluster_percentages2 <- cluster_percentages2 %>% mutate(Cluster = 
                                                        case_when(Cluster=="1" ~ "RodLike",
                                                                  Cluster=="2" ~ "Ramified",
                                                                  Cluster=="3" ~ "Ameboid"))
write.csv(final_df_live, "/Users/alexlawson/Masters-Data-Final/Live-imaging/Ciernia-Lab-Morphometrics/Results/livecelldataset.csv", row.names = FALSE)

View(final_df_live)

#ALL THE DATA MERGING
df_merged <- rbind(finaldata_live, new_df)
View(df_merged)
featurecorrelations(df_merged,
                     featurestart=8, featureend=34,
                     rthresh=0.8, pthresh=0.05,
                     title="Correlations across features")

df_merged_log <- transform_log(df_merged,1, start=8, end=34) 
pca_df_merged_log <- pcadata(df_merged_log, featurestart=8, featureend=34,
                           pc.start=1, pc.end=10)
pca_df_merged_log_transformed <- transform_scale(pca_df_merged_log, start=1, end=3) # scale pca data as input for k-means clustering
kmeans_pca_df_merged_log_transformed <- pca_df_merged_log_transformed[1:3]
data_kmeans_pca_df_merged_log_transformed <- kmeans(kmeans_pca_df_merged_log_transformed, centers=3)
final_df_all <- cbind(pca_df_merged_log[1:3], df_merged, as.data.frame(data_kmeans_pca_df_merged_log_transformed$cluster)) %>%
   rename(Cluster=`data_kmeans_pca_df_merged_log_transformed$cluster`) 
clusterfeatures(final_df_all, featurestart=11, featureend=37)
View(final_df_all)
write.csv(final_df_all, "/Users/alexlawson/Masters-Data-Final/Live-imaging/Ciernia-Lab-Morphometrics/Results/mergeddataset.csv", row.names = FALSE)


cluster_percentages <- clusterpercentage(final_df_all, "Cluster", Treatment, Sex, BrainRegion, MouseID)
View(cluster_percentages)
plot_clusters <- clusterplots(final_df_all, "PC1", "PC2")
plot_clusters
cluster_percentages <- cluster_percentages %>% mutate(Cluster = 
                                   case_when(Cluster=="1" ~ "RodLike",
                                             Cluster=="2" ~ "Ameboid",
...
filtered_df <- cluster_percentages[cluster_percentages$MouseID %in% c("AL47", "AL51", "AL55", "AL59", "AL61", "AL62", "AL64", "AL65", 
                                                                      "AL67", "AL73", "AL74", "AL75", "AL77", "AL78", "AL80", "AL83", "AL85"),]
filtered_df_raw <- final_df_all[final_df_all$MouseID %in% c("AL47", "AL51", "AL55", "AL59", "AL61", "AL62", "AL64", "AL65", 
                                                                      "AL67", "AL73", "AL74", "AL75", "AL77", "AL78", "AL80", "AL83", "AL85"),]

View(filtered_df_raw)
